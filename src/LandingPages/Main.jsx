...
function Main() {
  ...
  const href = changeForm ? "#login" : "#register";
  return (
    <div id="main" className="relative min-h-min lg:min-h-screen 3xl:min-h-max">
      <div className="fixed 3xl:static top-0 inset-x-0 z-50">
        <Nav />
        {/* <Navbar button={toogle} text={text} href={href} /> */}
      </div>
      {modal ? <PopupSuccess toogle={modalInactive} /> : null}
      <div className="w-full lg:flex lg:items-center">
        <div className="w-full static flex justify-between items-center">
          <img
            src={img}
            alt="image"
            className="hidden lg:block absolute 3xl:static bottom-[4%] z-[-1] w-[60] h-[90%] ml-[-5%] mb-[2.5%]"
          />
          ...
          <div className="absolute w-[45%] hidden lg:block right-[5%] bottom-[10%]">
            {form}
          </div>
        </div>
      </div>
      <img
        src={logo}
        alt="team-up"
        className="absolute z-20 -bottom-1.5 lg:bottom-[-2.5%] left-[4%] lg:left-[2%] w-[27%]"
      />
      <div className="hidden sm:block absolute bg-[#22A6B3] h-[4%] w-full bottom-0" />
      <svg
        id="tentang"
        className="sm:absolute sm:bottom-[2%] lg:h-[45vh lg:z-[-1]"
